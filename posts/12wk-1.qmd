...
# a, b의 그리드 생성
a_seq <- seq(0, 1.5, by = 0.1)
b_seq <- seq(1.5, 3, by = 0.1)

# 가능도 함수 계산을 위한 행렬 생성
likelihood_matrix <- matrix(0, nrow = length(a_seq), ncol = length(b_seq))
...
for(i in 1:length(a_seq)) {
    for(j in 1:length(b_seq)) {
        ...
        if(b_val > a_val && all(a_val <= x) && all(x <= b_val)) {
...
# 3D plot
persp(a_seq, b_seq, likelihood_matrix,
      theta = 30, phi = 30,
      xlab = "a", ylab = "b", zlab = "Likelihood",
